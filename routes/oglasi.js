...
router.post("/cena", async (req, res) => { //localhost:3000/oglasi/
    var arrayOfCoverImagePath = [];
    var arrayOfCoverImagePathKorisnici = [];
    ...
    try {
        ...
        const oglasi = await Oglas.find({}).populate("korisnikId");
         (req.body.cena !=null && req.body.cena !='')
        {
            oglasi=oglasi.lte('cena',req.body.cena)
        }

         oglasi = await oglasi.exec()
            console.log(oglasi)
        oglasi.forEach(oglas => {
            if(oglas.slike.length === 0){
                //console.log(oglas.cena + " je " + oglas.slike.length);
...
router.post("/date", async (req, res) => { //localhost:3000/oglasi/
    var arrayOfCoverImagePath = [];
    var arrayOfCoverImagePathKorisnici = [];
    try {
 
        let query=Oglas.find()
        if (req.body.datum !=null && req.body.datum !='')
        {
            query=query.lte('useljivOd',req.body.datum)
        }
...
router.post("/:id", async (req, res) => { //localhost:3000/oglasi/
    try {
        ...
        const korisnik = await Korisnik.findById(oglas.korisnikId)
        const nizGradova = await Grad.find({naziv: oglas.nazivGrada}).exec();
        if (req.body.komentar!=null && req.body.komentar!='')
        {
            ...
            await oglas.save()//mozda ovo zeza
        }
         if (req.user==null)
        {
            var user={
        ...
        oglas.ratingavg=(oglas.ratingavg+int)/oglas.ratings.length
        console.log(oglas.ratingavg)
    
        oglas.save()
        console.log(oglas.ratings)
        }
...
router.delete("/komentari/:id", async (req, res) => {
    ...
    try{
        ...
        oglas = await Oglas.findById(req.params.id);
        let komentarindex=req.body.index;
        oglas.komentari.splice(komentarindex,1) //Kada stavimo crticu ispred, idemo od roota. Da ne stavimo bilo bi authors/autors -> znaci bilo bi relativna putanja
        oglas.save()
